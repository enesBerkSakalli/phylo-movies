...
The browser demo uses the same direct Phylo-Movies payload contract as backend runs. The reviewer-facing norovirus, bootstrap, tree-search, quick MSA, and paper examples include generated interpolation frames in their bundled `.movie.json` payloads. The `msprime 1000-Taxa Limit Demo` is the only intentionally input-tree-only payload, because it is a maximum-taxon rendering check rather than an interpolation walkthrough.
