...
Reviewer-response prose should only use completed language for rows marked
`Closed`; rows with any other status should be described as pending final
manuscript, release, media, or smoke-test verification.
...
| Bootstrap/support filtering while retaining animation is unclear. | Bootstrap-aware branch-support annotations are parsed into the SPR movement ledger; the movement events table reports source/destination support and can filter movements with a user-selected support threshold, while the tree animation itself does not automatically suppress uncertain rearrangements. | `README.md`; `revision/MBE_response_to_reviewers_with_answers.md`; `revision/MBE_revision_response_draft.md`; `src/domain/tree/branchSupportIndex.js`; `src/domain/spr/sprAnalytics.js`; `src/components/TreeStatsPanel/SubtreeAnalytics/SprMoveEventTable.tsx`; `src/treeVisualisation/deckgl/layers/factory/labels/SupportLabelLayers.js`; `src/treeVisualisation/deckgl/layers/factory/LayerSetFactory.js`. | Static table check covers support filter and threshold wiring; documentation/code review confirms no animation-level support pruning is claimed. | Manuscript text needed | Mirror this support-aware table filtering distinction in the rogue-taxon/use-cases section. |
...
| Rogue-taxon source data are third-party and should not be confused with Phylo-Movies data. | Do not vendor the full Aberer/RogueNaRok archive; record only selected source files and checksums. | `publication_data/bootstrap_rogue_taxa/source_alignments/MANIFEST.tsv`; `publication_data/bootstrap_rogue_taxa/README.md`; `publication_data/PUBLICATION_ARCHIVE.md`. | SHA256 values recorded for source alignments `24` and `125`; selected files are included in the publication-data archive contract. | Closed | Use the archive contract for final release/DOI packaging. |
...
| Manuscript says all data are available in the repository. | The bioRxiv upload package was manuscript-only; repository/release archive must carry reproducibility data. | `publication_data/README.md`; `publication_data/PUBLICATION_ARCHIVE.md`; `publication_data/REGENERATE.md`; `publication_data/PUBLICATION_DATA_RELATIONSHIP_MAP.md`; `revision/BIOARXIV_UPLOAD_DATA_DISTRIBUTION_COMPARISON.md`. | Checksum verification passes for `bootstrap_rogue_taxa/current_results/MANIFEST.sha256` and `recombination_norovirus/MANIFEST.sha256`; archive-noise scan finds no `runs/`, `__pycache__`, `*.pyc`, or stale output payloads. | Closed | Use this archive contract for final release/DOI packaging and mirror it in the Data Availability statement. |
...
4. Add final Data Availability wording for the selected Aberer/RogueNaRok
   alignments and the hashed Nextstrain-derived norovirus snapshot.
5. Use the publication-data archive contract when preparing the final release or
   DOI deposit.
