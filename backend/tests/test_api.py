...
def test_treedata_valid_file(client):
    ...
    assert "interpolated_trees" in result
    assert "tree_names" in result
    assert "tree_count" in result
    assert "distances" in result
    assert "embedding" in result
    assert "sorted_leaves" in result
    assert "highlighted_elements" in result
    assert "lattice_edge_tracking" in result
    ...
    assert "original" in result["tree_count"]
    assert "interpolated" in result["tree_count"]
