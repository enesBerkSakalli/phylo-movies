...
def write_run_log(log_path, datasets, args, run_info):
    ...
    with open(log_path, "w") as f:
        ...
        f.write(
            "  - ranked/bootstrap_order_<name>.tsv: Ranked replicates by composition distance\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--source-root",
        default="/Users/berksakalli/Projects/rogue_taxa_analysis/online-data/data/datasets/alignments",
        help="Directory containing external Aberer/RogueNaRok alignment files.",
    )
    parser.add_argument(
        "--dataset",
        action="append",
    ...
    parser.add_argument(
        "--output-base",
        default=os.path.join("publication_data", "bootstrap_example", "iqtree_reinference", "runs"),
    ...
    for ds in datasets:
        ...
        out_file = os.path.join(ranked_dir, f"bootstrap_order_{output_label}.tsv")
        ...
        dataset_manifest = {
            ...
            "ranked_outputs": {
                "bootstrap_order": os.path.relpath(out_file, ds_dir),
                "bootstrap_order_sha256": sha256_file(out_file),
