...
SOURCE_ROOT="${ROGUE_TAXA_SOURCE_ROOT:-/Users/berksakalli/Projects/rogue_taxa_analysis/online-data/data/datasets/alignments}"
OUTPUT_BASE="${REPO_ROOT}/publication_data/bootstrap_example/iqtree_reinference/runs"
CONDA_ENV="${PHYLOMOVIES_RAXML_ENV:-epas1-pipeline}"
...
usage() {
  cat <<'EOF'
Usage:
  run_bootstrap_examples.sh [options]
...
Options:
  ...
  --seed N                Random seed.
  --dataset ID            Dataset under source root. Repeatable.
  --source-root DIR       Directory containing external alignment files.
  --output-base DIR       Base output directory.
  --run-label LABEL       Optional short label appended to the run directory name.
  --tree-program NAME     iqtree, fasttree, or raxml. Default: iqtree.
  --iqtree-mode MODE      default or fast. Default: default.
  --conda-env NAME        Conda env containing raxmlHPC. Default: epas1-pipeline.
