...
./run_bootstrap_rogue_taxa.sh --replicates 200 --tree-program iqtree --jobs 4 --threads 1
./run_bootstrap_rogue_taxa.sh --replicates 200 --tree-program iqtree --run-label reviewer-r3
```

FastTree remains available for explicit exploratory runs:

```bash
./run_bootstrap_rogue_taxa.sh --smoke --tree-program fasttree
```

Create the shared publication environment from the top-level publication data
file:
...
The shell entry point defaults to the `phylomovies-publication` conda environment so
RAxML, IQ-TREE, FastTree, and the Python dependencies resolve from one place.
Override with:

```bash
PHYLOMOVIES_PUBLICATION_ENV=my-env ./run_bootstrap_rogue_taxa.sh --smoke
```

The default source root is `publication_data/bootstrap_rogue_taxa/source_alignments`.
Override it with `BOOTSTRAP_SOURCE_ROOT` or `--source-root` for a deliberate
rerun against another source location.
...
```text
publication_data/bootstrap_rogue_taxa/runs/
```
