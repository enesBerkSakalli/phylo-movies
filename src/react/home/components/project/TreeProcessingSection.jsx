...
export function TreeProcessingSection({ hasTrees, hasMsa, disabled }) {
        ...
        render={({ field }) => (
          <FormItem className="rounded-lg border bg-muted/20 p-4 flex items-center justify-between gap-4 h-fit space-y-0 transition-colors hover:bg-muted/30">
            <div className="space-y-1">
              ...
              <FormDescription className="text-2xs leading-tight max-w-[160px]">
                Establish root at the branch that bisects the tree's diameter.
              </FormDescription>
            </div>
            <FormControl>
              <Switch
