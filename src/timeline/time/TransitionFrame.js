...
const normalizeOptionalProgress = (value) => (
    Number.isFinite(value) ? clamp01(value) : null
);

export class TransitionFrame {
    static from(frame = {}, options = {}) {
        const sourceTreeIndex = normalizeIndex(
            frame.sourceTreeIndex,
            0
        );
        const targetTreeIndex = normalizeIndex(
            frame.targetTreeIndex,
            sourceTreeIndex
        );
