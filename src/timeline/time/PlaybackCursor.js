...
export class PlaybackCursor {
  static fromPlayhead({ animationProgress = 0, timelineProgress = null, frameIndex = 0 } = {}) {
  ...
  static fromTransitionFrame(
    transitionFrame,
    { treeCount, timelineProgress = transitionFrame?.timelineProgress } = {}
  ) {
    const exactFrameIndex =
      transitionFrame.sourceTreeIndex +
      (transitionFrame.targetTreeIndex - transitionFrame.sourceTreeIndex) *
        transitionFrame.transitionProgress;
    ...
    const animationProgress =
      Number.isFinite(treeCount) && treeCount > 1 ? clamp01(exactFrameIndex / (treeCount - 1)) : 1;
  ...
  constructor({ point, animationProgress, timelineProgress, frameIndex, holdKind = null }) {
    this.point = point;
