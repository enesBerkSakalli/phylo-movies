...
export class TimelineDataProcessor {
    ...
    static _createSegmentsFromRows({
        ...
        this._appendInputTreeSegment(inputFrames[inputFrames.length - 1], frames, interpolatedTrees, segments);
    ...
    static _appendInputTreeSegment(frame, _frames, interpolatedTrees, segments) {
        segments.push(TimelineSegmentBuilder.buildInputTreeSegment({
    ...
    static _appendPairTransitionSegments({
        ...
        if (splitEvents.length === 0) {
            segments.push(TimelineSegmentBuilder.buildBranchLengthOnlySegment({
                segmentIndex: segments.length,
                pair,
        ...
        for (const event of splitEvents) {
            ...
            if (coveredThroughFrameIndex < gapEnd) {
                segments.push(TimelineSegmentBuilder.buildFulfillmentSegment({
                    segmentIndex: segments.length,
                    pair,
            ...
            segments.push(TimelineSegmentBuilder.buildSplitEventSegment({
                segmentIndex: segments.length,
                event,
                pair,
                sprEvents,
...
function isExactNoOpPair(pair, pairMetric) {
    return pair.generated_frame_range === null &&
