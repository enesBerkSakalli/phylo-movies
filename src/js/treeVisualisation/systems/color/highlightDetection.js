/**
 * Highlight detection utilities
 * Handles detection of active change edges and marked components
 */
 ...
 */
export function isActiveChangeEdgeHighlighted(linkData, activeChangeEdge) {
 ...
 */
export function isDownstreamOfActiveChangeEdge(linkData, activeChangeEdges) {
 ...
 * @returns {boolean} True if downstream
 */
export function isNodeDownstreamOfActiveChangeEdge(nodeData, activeChangeEdges) {
...
/**
 * Check if branch is part of marked subtree
 * @param {Object} linkData - D3 link data
 * @param {Array<Set<number>>} markedComponents - Array of marked component Sets
 * @returns {boolean} True if marked
 */
export function isComponentMarked(linkData, markedComponents) {
  if (!linkData?.target?.data?.split_indices || !markedComponents) {
    return false;
  }
...
/**
 * Check if node intersects with any marked subtree
 * @param {Object} nodeData - Node data
 * @param {Array<Set<number>>} markedComponents - Array of marked component Sets
 * @returns {boolean} True if marked
 */
export function isNodeMarked(nodeData, markedComponents) {
  if (!nodeData?.data?.split_indices || !markedComponents) {
    return false;
  }
