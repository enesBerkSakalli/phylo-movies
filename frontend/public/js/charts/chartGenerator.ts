...
export function generateLineChart(containerId, data, config, guiInstance) {
  ...
  function calculateChartDimensionsForContainer(containerId) {
    ...
    // Check if this chart is inside the movie player bar for special compact sizing
    const isInMoviePlayerBar = container.closest('.movie-player-bar') !== null;
    
    console.log(`[chartGenerator] Container ${containerId}:`, {
      isInMoviePlayerBar,
      containerWidth,
      containerHeight,
      hasMoviePlayerBarParent: !!container.closest('.movie-player-bar')
    });
    
    // Ensure minimum dimensions - use different minimums for movie player bar context
    const adjustedWidth = Math.max(containerWidth, 280);
    const adjustedHeight = isInMoviePlayerBar 
      ? containerHeight  // Use actual container height for movie player bar
      : Math.max(containerHeight, 140); // Default minimum for other contexts

    const margin = isInMoviePlayerBar 
      ? {
          // Minimal but functional margins for movie player bar
          top: 2,
          right: 5,
          bottom: 8,
          left: 35  // Enough space for y-axis labels to be readable
        }
      : {
          // Standard margins for other contexts
