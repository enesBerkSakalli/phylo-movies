...
import drawTree from "../treeVisualisation/TreeAnimationController.js";

// Load CSS
const cssLink = document.createElement('link');
cssLink.rel = 'stylesheet';
cssLink.href = '/css/tree-comparison.css';
...
export class TreeComparison {
  ...
  async renderComparison(params) {
    ...
    try {
      ...
      const tree1Layout = constructTree(treeList[tree1Index], ignoreBranchLengths, {
      ...
      await drawTree({
        treeConstructor: tree1Layout,
        toBeHighlighted: new Set(highlightData1),
        svgContainerId: tree1GroupId,
        ...renderOptions
      });

      // Render tree 2 with size constraints
      const tree2Layout = constructTree(treeList[tree2Index], ignoreBranchLengths, {
      ...
      await drawTree({
        treeConstructor: tree2Layout,
        toBeHighlighted: new Set(highlightData2),
        svgContainerId: tree2GroupId,
        ...renderOptions
      });
    } catch (error) {
  ...
  async renderInterpolationFrame(frame, svgId, options) {
    ...
    const treeLayout = constructTree(frame.tree, ignoreBranchLengths);
    try {
      await drawTree({
        treeConstructor: treeLayout,
        toBeHighlighted: new Set(toBeHighlighted),
        svgContainerId: treeGroupId,
        drawDurationFrontend: 200,
        fontSize,
        strokeWidth,
        leaveOrder
      });
