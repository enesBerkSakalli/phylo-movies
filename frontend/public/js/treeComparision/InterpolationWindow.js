...
export class InterpolationWindow {
      ...
      setTimeout(async () => {
        try {
          ...
          try {
            ...
            console.error('[InterpolationWindow] Tree drawing failed:', drawError);
  ...
  async animateInterpolation() {
    ...
    // Ensure minimum time between frames to allow animations to complete
    // The tree animation uses drawDuration (200ms in renderInterpolationStep)
    // We add a small buffer to ensure smooth transitions
    const minStepDuration = 250; // 200ms animation + 50ms buffer
    const effectiveStepDuration = Math.max(stepDuration, minStepDuration);
    ...
    for (let i = this.currentInterpolation.currentStep;
      ...
      if (i < sequence.length - 1) {
        await new Promise(resolve => setTimeout(resolve, effectiveStepDuration));
      }
    }
