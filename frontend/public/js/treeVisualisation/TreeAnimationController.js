...
export class TreeAnimationController {
  ...
  getArcInterpolationFunction() {
    ...
    return function (d) {
      ...
      // Validate path data - reject if null, undefined, or contains NaN
      if (!prev_d || prev_d.includes('NaN') || prev_d.includes('undefined')) {
        prev_d = null;
      }

      let pathArray = prev_d ? self.parser.parsePathData(prev_d) : null;

      // Additional validation: check if pathArray contains valid numbers
      if (pathArray && Array.isArray(pathArray)) {
        const isValid = pathArray.every(point =>
          point &&
    ...
    try {
      ...
      if (animationStrategy === 'exit_first') {
        ...
        if (transitionType?.isITDownToC) {
          // Special handling for IT_DOWN → C: simultaneous exit + update with complementary easing
          const simultaneousPromises = [
            // Exit and Update happen truly simultaneously using the same linkStages
          ...
          // Enter new elements after
          await linkStages.stage1();
        } else {
          // IT_UP → C: Also simultaneous exit + update (same as IT_DOWN → C)
          const simultaneousPromises = [
            // Exit and Update happen truly simultaneously using the same linkStages
            linkStages.stage3(), // Exit old elements
            linkStages.stage2(), // Update existing elements
        ...
        // C → IT_UP: Update first, then enter (sequential, not simultaneous)
        // This prevents creating new branches with zero length during animation
        ...
        // Enter new elements after update completes
        await linkStages.stage1();

      } else {
        // Default: Enter → Update → Exit sequence
      ...
      // Avoid logging D3 nodes or links directly (circular structure)
      console.error("[TreeAnimationController] Error in renderWithCoordinatedAnimations:", error, {
        ...
        animationStrategy: this.getAnimationStrategy(),
        linksCount: this.root?.links()?.length
      });
      throw error;
    }
  }
  ...
  getAnimationStrategy() {
    ...
    try {
      ...
      console.warn("[TreeAnimationController] Error determining animation strategy:", error);
      return 'default';
    }
  }
  ...
  calculateLayout(treeData, ignoreBranchLengths) {
    ...
    const layoutCalculator = new TreeConstructor(d3hierarchy, ignoreBranchLengths);
  ...
  static async draw({
    ...
    // Set transition tracking for phase-aware animations
    if (transitionResolver) {
      instance.setTransitionResolver(transitionResolver);
    }
    if (treeIndex !== undefined) {
      instance.updateTreeIndex(treeIndex);
    }
  ...
  renderInterpolatedFrame(fromTreeData, toTreeData, timeFactor, options = {}) {
    ...
    if (!fromTreeData || !toTreeData) {
      console.warn('[TreeAnimationController] Invalid tree data for interpolation:', {
        fromTreeData: !!fromTreeData,
