...
describe('unused plumbing cleanup', () => {
  ...
  it('does not keep the unused deck.gl layer factory barrel', () => {
    ...
    const supplementalLayerTest = readFileSync(
      join(repoRoot, 'test', 'mocha', 'supplemental', 'layer-highlighting.test.js'),
      'utf8'
    );

    expect(existsSync(barrelPath), relative(repoRoot, barrelPath)).toBe(false);
    expect(supplementalLayerTest).not.toContain('layers/factory/index.js');
  });
